...
@UriEndpoint(firstVersion = "3.8.0", scheme = "paho-mqtt5", title = "Paho MQTT 5",
             consumerClass = PahoMqtt5Consumer.class, label = "messaging,iot",
             syntax = "paho-mqtt5:topic")
public class PahoMqtt5Endpoint extends DefaultEndpoint {

    private static final Logger LOG = LoggerFactory.getLogger(PahoMqtt5Endpoint.class);
    ...
    public PahoMqtt5Endpoint(String uri, String topic, Component component) {
        super(uri, component);
        this.topic = topic;
    }
    ...
    public Producer createProducer() throws Exception {
        return new PahoMqtt5Producer(this);
    }

    @Override
    public Consumer createConsumer(Processor processor) throws Exception {
        return new PahoMqtt5Consumer(this, processor);
    }
    ...
    protected MqttClientPersistence resolvePersistence() {
        if (persistence ==  PahoMqtt5Persistence.MEMORY) {
    ...
    protected MqttConnectionOptions resolveMqttConnectOptions() {
        if (connectionOptions != null) {
            return connectionOptions;
        }
        
        if (resolveMqttConnectOptions) {
            Set<MqttConnectionOptions> connectOptions = getCamelContext().getRegistry().findByType(MqttConnectionOptions.class);
     ...
     */
    public void setConnectionOptions(MqttConnectionOptions connOpts) {
        this.connectionOptions = connOpts;
    }
